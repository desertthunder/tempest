defmodule Tempest.Blobs.S3StorageTest do
  ...
  test "promote_blob falls back to read and write when copy cannot find the source", %{
    config: config,
    did: did,
    cid: cid
  } do
  ...
  test "promote_blob reports missing only when fallback read cannot find the temp object", %{
    config: config,
    did: did,
    cid: cid
  } do
